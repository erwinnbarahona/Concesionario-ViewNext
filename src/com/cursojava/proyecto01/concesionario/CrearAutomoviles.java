...
/*<<<<<<<<<<<<<METODOS PARA CREAR MOTOS>>>>>>>>>>>>>>>>>>>>*/
	...
	/*<<<<<<<<<<<<< FINAL METODOS PARA CREAR MOTOS>>>>>>>>>>>>>>>>>>>>*/
/*<<<<<<<<<<<<<METODOS PARA CREAR DEPORTIVOS>>>>>>>>>>>>>>>>>>>>*/
	
	protected static Deportivo crearDeportivo() {
		Deportivo deportivo;
		numPlaca = elegirPlacas();
		nomAutomoviles = nombreMoto();
		numCaballos = numCaballosMoto();
		pesoAutomoviles = pesoMoto();
		nombreEquipo = elegirNombreEquipo();
		deportivo = new Deportivo(NUM_RUEDAS_COCHE, numPlaca, numCaballos, nombreEquipo);
		
		return deportivo;
	}
	
	protected static String nombreDeportivo() {
		String [] nombre = {"CfMoto 125NK", "Sym Adxtg", "Honda CDB"
				, "Voge 300"};
		Random random = new Random();
	...
	protected static double persoDeportivo() {
		double [] pesoCoche = {250.0, 260.0, 170.0};
		Random random = new Random();
	...
	protected static int numCaballosDeportivo() {
		int [] numCaballos = {375, 380, 385};
		Random random = new Random();
	...
	protected static String elegirNombreEquipo() {
		String [] nombre = {"Mercedes AMG", "Ferrari", "Red Bull"
				, "Mc laren"};
		Random random = new Random();
	...
	/*<<<<<<<<<<<<< FINAL METODOS PARA CREAR MOTOS>>>>>>>>>>>>>>>>>>>>*/
	/*<<<<<<<<<<<<< METODOS PARA CREAR PLACAS DE AUTOMOVILES>>>>>>>>>>>>>>>>>>>>*/
